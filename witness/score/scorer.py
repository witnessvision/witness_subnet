"""Pure, deterministic reconstruction scoring through scene.json v1.5."""
...
def _f_score(
    ...
    *,
    ...
    if matches == 0:
        return 0.0
    false_positives = max(0, prediction_count - matches)
    prediction_count = matches + min(false_positives, FALSE_POSITIVE_CAPS[family])
...
def _qa_match(answer: Any, truth: Any) -> float:
    """Lenient QA credit: exact normalized match, truth contained in answer, or token-F1 >= 0.6."""
    predicted = normalize_answer(answer)
    expected = normalize_answer(truth)
    if not expected:
        return 1.0 if not predicted else 0.0
    if predicted == expected or f" {expected} " in f" {predicted} ":
        return 1.0
    p_tokens, e_tokens = predicted.split(), expected.split()
    if not p_tokens:
        return 0.0
    common = 0
    remaining = list(e_tokens)
    ...
    if common == 0:
        return 0.0
    precision, recall = common / len(p_tokens), common / len(e_tokens)
    f1 = 2 * precision * recall / (precision + recall)
    return 1.0 if f1 >= 0.6 else 0.0
...
def score_reconstruction(
    ...
    *,
    ...
    report = {
        ...
        "false_positive_caps": FALSE_POSITIVE_CAPS.copy(),
